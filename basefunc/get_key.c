...
key* get_key(enum curve_name ecname)
{
	...
	mpz_mod(key, key, c->n);
	newkey = malloc(sizeof(key) + sizeof(char) *((c->length / 4 + 1) * 3 + 10));
	newkey->curve = malloc(sizeof(char) * 10);
	newkey->private = malloc(sizeof(char)*(c->length / 4 + 1));
	newkey->public_x = malloc(sizeof(char)*(c->length / 4 + 1));
	newkey->public_y = malloc(sizeof(char)*(c->length / 4 + 1));
